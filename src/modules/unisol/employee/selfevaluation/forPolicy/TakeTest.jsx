...
const TakeTest = () => {
    ...
    const { fetchPolicyQuestions, policyQuestions, takePolicyTest,
        loading, policyQAPreview, resetPolicyQAPreview
    } = useSelfEvaluation();
    ...
    console.log("policyQuestions:", policyQuestions,
        "policyQAPreview:", policyQAPreview
    );

    const questions = policyQuestions?.questions

    useEffect(() => {
        if (policyId) fetchPolicyQuestions(policyId);
    }, [policyId]);
    ...
    const handleSubmit = async (values) => {
        ...
        try {
            ...
            const isSuccess = await takePolicyTest(payload, policyId)
            if (isSuccess) {
                console.log("isSuccess", isSuccess);
                navigator('/emp/selfevaluation/policy/taketest/preview', {
    ...
    return (
        <div>
            <Breadcrumb linkText={[
                { text: "Self Evaluation", href: "/emp/selfevaluation/policy", onClick: () => resetPolicyQAPreview() },
                { text: "Take Test" }
            ]} />
            <div className='flex flex-col bg-gray-100 min-h-screen'>
                        ...
                        <div className="flex items-center space-x-3">
                            ...
                            <div>
                                <h1 className="text-2xl font-bold">
                                    {loading ? "Loading..." : policyQuestions?.moduleTitle}
                                </h1>
                            </div>
                        </div>
                        {/* Timer */}
                ...
                >
                    {({ values, handleChange, handleBlur, errors, touched }) => (
                        <Form className='bg-white shadow-md p-8 mx-auto w-full'>
                            {paginatedQuestions?.map((question, idx) => (
                                <div
                                    key={question._id}
                                    className="flex flex-col gap-6 mb-8 p-6 border border-gray-300 border-l-[6px] rounded-xl shadow-md last:mb-0"
                                    style={{
                    ...
                    <div className="bg-white rounded-lg shadow-lg p-8 w-full max-w-md text-center space-y-6">
                        ...
                        <div className="flex justify-center gap-4">
                            ...
                            <button
                                onClick={() => {
                                    resetPolicyQAPreview();
                                    navigator("/emp/selfevaluation/policy");
                                }}
